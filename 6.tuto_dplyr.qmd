...
Create a column named `none.diag` containing a `TRUE` if the column `diagnosis` contains `No Disease` and `FALSE` otherwise and display the 10 rows containing the lowest MAPK signature values and the columns `patient_id`, `diagnosis` , `heart_rate` and the new column `none.diag`.
