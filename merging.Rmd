...
To download it, you just need to type : 
...
|----------|----------|----------|
| *starts_with()* | Starts with a specific character/string | **storms %>% select(starts_with("l"))** |
| *ends_with()* | Ends with a specific character/string | **storms %>% select(ends_with("diameter"))** |
| *contains()* | Contains a specific character/string | **storms %>% select(contains("on"))** |
...
Example : Operation on a column, and put result into new columns

```{r mutate example}
storms %>%
  mutate(example = wind / 10)
```

As you can see, we just divided the wind column by 10 and put the result in new column named "example".
